...
class Data {
    ...
    func saveGame(main: MainModel) {
        ...
        saveErrorCheck(main.errorCheck!)
    ...
    func setLoadMode(mode: Bool) {
        storage.set(mode, forKey: Data.LOADMODE)
    }
    
    func getLoadMode() -> Bool {
        return storage.bool(forKey: Data.LOADMODE)
    }
        ...
        for i in 0..<fields.count {
            for j in 0..<fields[i].count {
                for (index, field) in fields[i][j].enumerated() {
                    storage.set(field.isNotes, forKey: Data.FIELD_ISNOTES + String(i) + String(j) + String(index))
        ...
        for i in 0..<9 {
            ...
            for j in 0..<3 {
                ...
                for a in 0..<3 {
                    ...
                    main.fields[i][j][a].isNotes = storage.bool(forKey: Data.FIELD_ISNOTES + String(i) + String(j) + String(a))
                    if main.fields[i][j][a].isNotes {
                        ...
                        print(String(storage.integer(forKey: Data.FIELD_NUMBER + String(i) + String(j) + String(a))))
    ...
    private func saveErrorCheck(_ errorCheck: ErrorCheck) {
        storage.set(errorCheck.overallErrors, forKey: Data.ERRORCHECK_OVERALLERRORS)
    }
    ...
    private func loadErrorCheck(_ main: MainModel) {
        ...
        main.errorCheck!.overallErrors = storage.integer(forKey: Data.ERRORCHECK_OVERALLERRORS)
    }
}
